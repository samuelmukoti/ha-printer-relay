...
### Changed
- **Tunnel-based authentication** - When tunnel is enabled, all API requests are allowed
  - Tunnel URL acts as a secret - only users with HA access know it
  - Removes need for OAuth/bearer tokens when using tunnel
  - Mobile app can connect directly with just the tunnel URL

### Fixed
- **Mobile app 401 errors** - Fixed authentication requirement blocking mobile app requests via tunnel
...
### Fixed
- **Schema validation error** - Changed tunnel config schema to use `str?` instead of `url?`
  - Empty string for `tunnel_url` was failing URL validation
  - Made all tunnel fields optional with `?` suffix
